		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
					...
					if(head!= null) {
						...
						if(head == '99') {
							...
							if(user_duty == '19') {
								...
								console.log("if(user_duty == '19') 진입 >>> ");
								
									$('#selectedMemberList').append("<div>배나영(이사)(H202002190000, 임원/ 대표이사)</div><br>"); //이사 값 바뀌면 수정
								...
								console.log("if(user_duty == '19')-else 진입 >>> ");
								
								alert("다른 임직원을 선택해야 합니다.");
				...
				$('#teams').change(function() {
					...
					//사원이 같은 부서의 팀장을 선택하지 않았을 때
					if((user_position == '17') && (deptnum != user_deptnum)) {
						console.log("if((user_position == '17') && (deptnum != user_deptnum)) 진입 >>> ");
						
						alert("휴가신청서는 같은 팀이여야 합니다.");
						
					} else if((user_duty == '18') && (deptnum != user_deptnum.substring(0,2))) { //팀장이 같은 본부의 본부장을 선택하지 않았을 때
						console.log("else if((user_position == '18') && (deptnum != user_deptnum)) 진입 >>> ");
						
						alert("휴가신청서는 같은 본부여야 합니다.");
						console.log("user_deptnum.substring(0,2) >>> : " + user_deptnum.substring(0,2));
						console.log("deptnum >>> : " + deptnum);
						
					} else if((user_duty == '19') && (deptnum != '98')) { //본부장이 대표이사를 선택하지 않았을 때
						console.log("else if((user_duty == '19') && (deptnum != user_deptnum)) 진입 >>> ");
						
						alert("휴가신청서는 대표이사를 선택해야 합니다.");
						
					} else {
						console.log("if((user_position == '17') && (deptnum != user_deptnum))-else 진입 >>> ");
				...
				$('#members').change(function() {
					...
					if(user_position == '17') {
						...
						console.log("if(user_position == '17') 진입 >>> ");
						
						if(member_name.indexOf('팀장') != -1) { //사원이 임직원 선택에서 팀장을 선택했을 때 
							console.log("if(member_name.indexOf('팀장') != -1) 진입 >>> ");
						
							$('#selectedMemberList').append("<div>" + member_name + "(" + member + ", " + head_name + "/ " + team_name + ")</div><br>");
						...
						} else if(member_name.indexOf('팀장') == -1) { //사원이 임직원 선택에서 팀장을 선택하지 않았을 때
							console.log("else if(member_name.indexOf('팀장') == -1) 진입 >>> ");
							
							alert("사원은 팀장을 선택해야 합니다.");
					...
					if(user_duty == '18') {
						...
						console.log("if(user_duty == '18') 진입 >>> ");
						
						if(member_name.indexOf('본부장') != -1) { //팀장이 임직원 선택에서 본부장을 선택했을 때 
							console.log("if(member_name.indexOf('본부장') != -1) 진입 >>> ");
						
							$('#selectedMemberList').append(member_name + "(" + member + ", " + head_name + ")<br>");
						...
						} else if(member_name.indexOf('본부장') == -1) { //팀장이 임직원 선택에서 본부장을 선택하지 않았을 때
							console.log("else if(member_name.indexOf('본부장') == -1) 진입 >>> ");
							
							alert("팀장은 본부장을 선택해야 합니다.");
				...
				$('#ok_button').click(function() {
					...
					if(line.length != 27) {
						console.log("if(line.length < 27) 진입 >>> ");
						
						alert("결재자는 총 1명이어야 합니다.");
						line = '';
						console.log("line >>> : " + line);
						auth_line = '';
						console.log("auth_line >>> : " + auth_line);
						...
						console.log("if(line.length < 27)-else 진입 >>> ");
